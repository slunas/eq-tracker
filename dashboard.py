...
def item_link(name):
    slug = name.replace(' ', '+')
    url = f'http://lucy.allakhazam.com/item.html?name={slug}&source=Live&exactname=on'
                ...
                with tab1:
                    ...
                    if listings:
                        ...
                        for r in listings:
                            ...
                            trows.append([typ, f'<a href="http://lucy.allakhazam.com/item.html?name={selected.replace(" ", "+")}&source=Live&exactname=on" target="_blank" style="color:#7ab8d8;text-decoration:none;">{selected}</a>', pp_str, kr_str, time_str])
